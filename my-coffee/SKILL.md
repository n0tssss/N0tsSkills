...
0. **switchProduct `operation` 参数未文档化** — `attrOperationParam.subAttr.operation` 要求 Integer 类型，但有效值不明（0/1/2 均返回"非法参数"）。**当前变通**：如果用户要冰美式等需要切换温度的商品，先用 `queryProductDetailInfo` 查看是否有独立 SKU（如 `searchProductForMcp` 返回多个 SKU 时，直接选目标 SKU），绕过 switchProduct。如果只有默认 SKU，用默认规格下单并告知用户当前规格，让用户决定是否继续。
1. **Hermes Token 掩码问题** — 终端工具会自动将 token 替换为 `***`，导致 bash 变量拼接的 curl 命令失效。**解决**：用 Python subprocess 从文件读取 token，避免 bash 层面的变量替换。参考写法：`token = open("/Users/wkea/.my-coffee/LUCKIN_MCP_TOKEN").read().strip()`。
2. 需要附近门店时，优先让用户提供位置、商圈、门店名或经纬度，再调用 `queryShopList`。
